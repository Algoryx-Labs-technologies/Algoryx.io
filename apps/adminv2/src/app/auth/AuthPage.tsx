...
import { User, Lock, KeyRound, Eye, EyeOff, LogIn, AlertCircle, Shield } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white transition-colors duration-300 flex items-center justify-center p-4">
      <div className="relative w-full max-w-2xl">
        ...
        <Card className="group relative bg-gradient-to-br from-slate-900/70 to-slate-800/50 backdrop-blur-sm border border-white/10 rounded-2xl shadow-lg overflow-hidden z-10">
          <CardHeader className="space-y-3 text-center px-8 pt-8">
            ...
            <CardTitle className="text-3xl font-semibold font-hero text-white">
              Admin sign in
            </CardTitle>
            <CardDescription className="text-base font-footer text-gray-400">
              Enter your Admin ID, password, and MPIN
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="px-8 pb-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                <Label htmlFor="mpin" className="font-footer text-gray-300">
                  MPIN
                </Label>
                <div className="relative">
                  <Input
                    ...
                    value={formData.mpin}
                    onChange={(e) =>
                      setFormData((prev) => ({ ...prev, mpin: e.target.value }))
                    }
                    required
                    className="pl-10 pr-10 h-11 font-footer bg-slate-800/80 border-white/5 text-white placeholder:text-gray-500 focus:border-blue-500/50"
                  />
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full h-11 font-footer bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 disabled:opacity-50"
                size="lg"
              >
                {loading ? (
                  'Signing in…'

...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (
    ...
    // Call API endpoint to create user and Admin record in database
    const response = await apiClient.post<{ user: any; session: Session }>('/auth/admin/signup', {
      email,
      password,
  ...
  const signIn = async (email: string, password: string) => {
    // Call API endpoint to sign in and validate admin role
    const response = await apiClient.post<{ user: any; session: Session }>('/auth/admin/signin', {
      email,
      password,
    });
    ...
    if (!response.success || !response.data) {
      return {
        user: null,
        session: null,
        error: { message: response.error || 'Failed to sign in' } as AuthError,
      };
    }

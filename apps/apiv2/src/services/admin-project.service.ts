...
export interface ListAdminProjectsOptions {
  stage?: AdminProjectStage;
  excludeStage?: AdminProjectStage;
  search?: string;
}
...
export const listAdminProjects = async (
  ...
  } else if (options.excludeStage) {
    if (!ADMIN_PROJECT_STAGES.includes(options.excludeStage)) {
      throw new AppError(400, 'Invalid project stage exclusion');
    }
    filter.stage = { $ne: options.excludeStage };
  }

      ...
      <CardContent className="px-4 pb-4 flex-1 overflow-y-auto">
        <div className="space-y-2">
          {newsItems.map((news, index) => (
            <div
              key={index}
              className="flex flex-col gap-1 p-2 rounded-lg bg-slate-800/50 border border-white/5 hover:bg-slate-800/70 transition-colors cursor-pointer"
            >
              <p className="text-xs text-white font-footer font-medium">
                {news.title}
              </p>
              <div className="flex items-center gap-2 text-xs text-gray-400 font-footer">
